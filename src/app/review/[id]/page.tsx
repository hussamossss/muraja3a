...
export default function ReviewPage() {
  ...
  async function handleConfirmReading() {
    ...
    if (page.last_read_at === today) {
      setReadingError(t('rvReadAlreadyErr'))
      return
    }
    setSaving(true)
    setReadingError('')
    try {
      ...
      if (le) {
        ...
        throw new Error(`${t('rvReadAlreadyErr')}: ${le.message}`)
      }
      ...
      setReadingError(err instanceof Error ? err.message : 'Error')
      setSaving(false)
    }
  }
  ...
  async function handleConfirmWords(forceZero = false) {
    if (!page) return
    if (selectedWords.length === 0 && !forceZero) { setConfirmZero(true); return }
    setSaving(true)
    ...
    try {
      ...
      if (selectedWords.length > 0) {
        const { error: we } = await saveWordMistakes({ userId: page.user_id, pageId: page.id, pageNumber: page.page_number, reviewLogId: saved.logId, selectedWords, allWords })
  ...
  // ── Quick mode options (inside component to access t()) ────────────────────
  const options: { key: MistakeLevel; label: string; emoji: string; desc: string; color: string; bg: string }[] = [
    { key:'perfect',   label:t('optPerfect'),   emoji:'🌟', desc:t('optPerfectDesc'),   color:'#22C55E', bg:'#0a1f10' },
    { key:'minor',     label:t('optMinor'),     emoji:'✅', desc:t('optMinorDesc'),     color:'#84CC16', bg:'#0d1a04' },
    { key:'impactful', label:t('optImpactful'), emoji:'⚠️', desc:t('optImpactfulDesc'), color:'#F97316', bg:'#1a1005' },
    { key:'few',       label:t('optFew'),       emoji:'🔸', desc:t('optFewDesc'),       color:'#FB923C', bg:'#1a1108' },
    { key:'many',      label:t('optMany'),      emoji:'❌', desc:t('optManyDesc'),       color:'#EF4444', bg:'#1a0808' },
    { key:'lapse',     label:t('optLapse'),     emoji:'🔄', desc:t('optLapseDesc'),     color:'#7C3AED', bg:'#130a1f' },
  ]
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', background:'var(--bg)' }}>
      ...
      <div style={{ background:'var(--bg)', padding:'24px 16px 16px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:12 }}>
        <button onClick={() => (reviewMode && !isReading) ? setReviewMode(null) : router.back()} style={backBtn}>‹</button>
        <span style={{ fontSize:17, fontWeight:700, color:'var(--cream)' }}>
          {reviewMode === 'words'   ? t('rvTitleWords')
          : reviewMode === 'reading' ? t('rvTitleReading')
          :                            t('rvTitleDefault')}
        </span>
      ...
      <div style={{ flex:1, padding:'20px 16px 40px', overflowY:'auto' }}>
        ...
        <div style={{ background:'#161A18', border:'1px solid var(--border)', borderTop:'3px solid #38BDF8', borderRadius:20, padding:'28px 20px', textAlign:'center', marginBottom:24 }}>
          ...
          <div style={{ fontSize:13, color:'var(--sub)', marginTop:6 }}>{t('pageWord')} · {t('detReviewCount')} {page.review_count}</div>
          <div style={{ display:'inline-block', marginTop:12, background:'rgba(56,189,248,0.12)', color:'#38BDF8', fontSize:12, fontWeight:700, padding:'4px 14px', borderRadius:20 }}>
            {page.current_interval_days} {t('daysWord')}
          </div>
        </div>
        ...
        {!reviewMode && (
          <>
            <div style={{ fontSize:13, fontWeight:600, color:'var(--sub)', marginBottom:12, textAlign:'center' }}>{t('rvModeQ')}</div>
            <div style={{ display:'flex', gap:10, marginBottom:8 }}>
              <button onClick={() => setReviewMode('quick')} style={{ flex:1, padding:'18px 12px', borderRadius:14, cursor:'pointer', border:'1.5px solid var(--border)', background:'#0F1210', display:'flex', flexDirection:'column', alignItems:'center', gap:8, fontFamily:'Amiri, serif', transition:'all .15s' }}>
                <span style={{ fontSize:28 }}>⚡</span>
                <span style={{ fontSize:15, fontWeight:700, color:'var(--cream)' }}>{t('rvModeQuick')}</span>
                <span style={{ fontSize:11, color:'var(--sub)' }}>{t('rvModeQuickDesc')}</span>
              </button>
              <button onClick={() => setReviewMode('words')} style={{ flex:1, padding:'18px 12px', borderRadius:14, cursor:'pointer', border:'1.5px solid var(--border)', background:'#0F1210', display:'flex', flexDirection:'column', alignItems:'center', gap:8, fontFamily:'Amiri, serif', transition:'all .15s' }}>
                <span style={{ fontSize:28 }}>🔤</span>
                <span style={{ fontSize:15, fontWeight:700, color:'var(--cream)' }}>{t('rvModeWords')}</span>
                <span style={{ fontSize:11, color:'var(--sub)' }}>{t('rvModeWordsDesc')}</span>
              </button>
            </div>
          </>
        )}
        ...
        {reviewMode === 'quick' && (
          <>
            <div style={{ fontSize:13, fontWeight:600, color:'var(--sub)', marginBottom:12, textAlign:'center' }}>{t('rvQuickQ')}</div>
            <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:18 }}>
              {options.map(opt => {
                const sel = mistakeLevel === opt.key
                return (
                  <button key={opt.key} onClick={() => setMistakeLevel(opt.key)} style={{ background: sel ? opt.bg : '#0F1210', border:`1.5px solid ${sel ? opt.color : 'var(--border)'}`, borderRight:`4px solid ${sel ? opt.color : 'transparent'}`, borderRadius:14, padding:'18px 16px', cursor:'pointer', display:'flex', alignItems:'center', gap:14, fontFamily:'Amiri, serif', width:'100%', minHeight:70, transition:'all .15s' }}>
            ...
            {quickResult && selectedOpt && (
              <div style={{ background:`${selectedOpt.color}10`, border:`1px solid ${selectedOpt.color}30`, borderRadius:14, padding:'16px 20px', textAlign:'center', marginBottom:16 }}>
                <div style={{ fontSize:12, color:'var(--sub)', marginBottom:4 }}>{t('rvNextLabel')}</div>
                <div style={{ fontSize:32, fontWeight:800, color: selectedOpt.color, lineHeight:1 }}>{quickResult.newInterval}</div>
                <div style={{ fontSize:12, color:'var(--sub)', marginTop:2 }}>{t('rvDaysFromNow')}</div>
              </div>
            )}
            <button onClick={handleConfirm} disabled={!mistakeLevel || saving} style={{ background: mistakeLevel ? (selectedOpt?.color ?? '#22C55E') : '#252B28', border:'none', color: mistakeLevel ? '#000' : 'var(--sub)', padding:16, borderRadius:14, cursor: mistakeLevel ? 'pointer' : 'not-allowed', fontSize:16, fontWeight:800, width:'100%', fontFamily:'Amiri, serif', transition:'all .15s' }}>
              {saving ? t('savingBtn') : mistakeLevel ? `${t('cancel').replace('إلغاء','تأكيد').replace('Cancel','Confirm')} — ${selectedOpt?.label}` : t('rvChooseFirst')}
            </button>
          </>
        )}
          ...
          <>
            {selectedWords.length > 0 && (
              <div style={{ background:'rgba(239,68,68,0.07)', border:'1px solid rgba(239,68,68,0.25)', borderRadius:14, padding:'12px 16px', marginBottom:16 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                  <span style={{ fontSize:11, fontWeight:600, color:'#EF4444' }}>{t('rvSelectedWords')}</span>
                  <button onClick={() => { setSelectedKeys(new Set()); setSelectedWords([]) }} style={{ background:'none', border:'none', color:'var(--sub)', fontSize:12, cursor:'pointer', fontFamily:'Amiri, serif' }}>
                    {t('rvClear')}
                  </button>
                </div>
                <div style={{ display:'flex', flexWrap:'wrap', gap:6, direction:'rtl' }}>
                  {selectedWords.map(w => (
                    <span key={`${w.s}:${w.a}:${w.wi}`} onClick={() => handleWordToggle(w)} style={{ fontFamily:'"Amiri Quran", serif', fontSize:18, color:'#EF4444', background:'rgba(239,68,68,0.12)', padding:'3px 10px', borderRadius:20, cursor:'pointer' }}>
                      {w.t}
                    </span>
                  ))}
                </div>
            ...
            {confirmZero && (
              <div style={{ background:'rgba(56,189,248,0.07)', border:'1px solid rgba(56,189,248,0.25)', borderRadius:14, padding:'16px', marginBottom:16, textAlign:'center' }}>
                <div style={{ fontSize:14, color:'var(--cream)', marginBottom:12 }}>{t('rvZeroQ')}</div>
                <div style={{ display:'flex', gap:10 }}>
                  <button onClick={() => handleConfirmWords(true)} style={{ flex:1, padding:'10px', borderRadius:10, cursor:'pointer', background:'#22C55E', border:'none', color:'#000', fontSize:14, fontWeight:700, fontFamily:'Amiri, serif' }}>{t('rvZeroYes')}</button>
                  <button onClick={() => setConfirmZero(false)} style={{ flex:1, padding:'10px', borderRadius:10, cursor:'pointer', background:'transparent', border:'1px solid var(--border)', color:'var(--sub)', fontSize:14, fontFamily:'Amiri, serif' }}>{t('cancel')}</button>
                </div>
              </div>
            )}

            <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:16, marginBottom:16, overflow:'hidden' }}>
              <QuranPage pageNumber={page.page_number} interactive selectedKeys={selectedKeys} onWordToggle={handleWordToggle} />
            </div>

            {!confirmZero && (
              <button onClick={() => handleConfirmWords()} disabled={saving} style={{ background: saving ? '#252B28' : '#EF4444', border:'none', color: saving ? 'var(--sub)' : '#fff', padding:16, borderRadius:14, cursor: saving ? 'not-allowed' : 'pointer', fontSize:16, fontWeight:800, width:'100%', fontFamily:'Amiri, serif', transition:'all .15s' }}>
                {saving
                  ? t('savingBtn')
                  : selectedWords.length > 0
        ...
        {reviewMode === 'reading' && (() => {
          ...
          return (
            <>
              <div style={{ background:'rgba(56,189,248,0.07)', border:'1px solid rgba(56,189,248,0.25)', borderRadius:14, padding:'14px 16px', marginBottom:16, textAlign:'center' }}>
                <div style={{ fontSize:13, color:'#38BDF8', fontWeight:700, marginBottom:4 }}>{t('rvReadingWarning')}</div>
                <div style={{ fontSize:12, color:'var(--sub)', lineHeight:1.7 }}>{t('rvReadingWarnSub')}</div>
              </div>
              <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:16, marginBottom:16, overflow:'hidden' }}>
                <QuranPage pageNumber={page.page_number} />
              </div>
              {readingError && (
                <div style={{ background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:12, padding:'12px 16px', marginBottom:12, fontSize:13, color:'#EF4444', textAlign:'center', fontFamily:'Amiri, serif' }}>
                  {readingError}
                </div>
              )}
              <button onClick={handleConfirmReading} disabled={disabled} style={{ background: disabled ? '#252B28' : '#38BDF8', border:'none', color: disabled ? 'var(--sub)' : '#000', padding:16, borderRadius:14, cursor: disabled ? 'not-allowed' : 'pointer', fontSize:16, fontWeight:800, width:'100%', fontFamily:'Amiri, serif', transition:'all .15s' }}>
                {saving ? t('savingBtn') : alreadyReadToday ? t('rvReadAlready') : t('rvReadDone')}
              </button>
            </>
          )
        })()}
