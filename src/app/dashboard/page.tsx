...
function useDateLabel() {
  const { lang } = useLang()
  const t = useT()
  return function dateLabel(dateStr: string): string {
    const d    = daysDiff(dateStr)
    const date = new Date(dateStr + 'T00:00:00')
    const dayName = date.toLocaleDateString(t.dates.locale, { weekday: t.dates.weekday })
    if (d === 1) return `${t.dates.tomorrow} · ${dayName}`
    if (d === 2) return `${t.dates.inTwoDays} · ${dayName}`
    if (d <= 7)  return `${dayName} · ${t.dates.inDays(d)}`
    return formatDate(dateStr)
  }
}

const MISTAKE_LABEL_AR: Record<MistakeLevel, string> = {
  perfect:'لا أخطاء', minor:'خطأ بسيط', impactful:'خطأ مؤثر',
  few:'2-3 أخطاء', many:'4-6 أخطاء', lapse:'نسيت',
}
const MISTAKE_LABEL_EN: Record<MistakeLevel, string> = {
  perfect:'No errors', minor:'Minor error', impactful:'Impactful error',
  few:'2–3 errors', many:'4–6 errors', lapse:'Forgotten',
}
...
function useStrengthInfo() {
  const { lang } = useLang()
  return function strengthInfo(s: Strength | null): { label: string; color: string } {
    if (lang === 'en') {
      switch (s) {
        case 'strong': return { label:'Strong', color: C.green  }
        case 'medium': return { label:'Medium', color: C.orange }
        case 'weak':   return { label:'Weak',   color: C.red    }
        default:       return { label:'New',    color: C.sub    }
      }
    }
    switch (s) {
...
export default function DashboardPage() {
  ...
  const strengthInfo = useStrengthInfo()
  ...
  const MISTAKE_LABEL = lang === 'en' ? MISTAKE_LABEL_EN : MISTAKE_LABEL_AR
    ...
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', background: C.bg, paddingBottom:180 }}>
      ...
      <div style={{ padding:'10px 20px 20px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:8 }}>
          ...
          <span style={{ fontSize:30, fontWeight:700, color: C.title }}>{t.dashboard.todayReviews}</span>
        </div>
          ...
          <div style={{ display:'flex', gap:8, paddingRight:16, marginBottom:12 }}>
            {dueToday.length > 0 && (
              <span style={{ background:`${C.green}18`, color: C.green, fontSize:12, fontWeight:700, padding:'5px 14px', borderRadius:20 }}>
                {dueToday.length} {t.dashboard.today}
              </span>
            )}
            {overdue.length > 0 && (
        ...
        {nextDate && (
          <button
            ...
            <span style={{ fontSize:13, color: C.accent, fontWeight:600 }}>
              {t.dashboard.nextSession(dateLabel(nextDate), nextCount)}
            </span>
      ...
      <div style={{ flex:1 }}>
        {total === 0 ? (
          <div style={{ textAlign:'center', padding:'60px 20px' }}>
            ...
            <div style={{ fontSize:20, fontWeight:700, color: C.title, marginBottom:8 }}>{t.dashboard.allDone}</div>
            <div style={{ fontSize:14, color: C.sub }}>{t.dashboard.nothingLeft}</div>
          </div>
        ) : (
          <>
            {dueToday.map(p => <ReviewRow key={p.id} page={p} isOverdue={false} router={router} t={t} getLastLabel={getLastLabel}/>)}
            {overdue.length > 0 && (
              <>
                <div style={{ display:'flex', alignItems:'center', gap:7, padding:'20px 20px 12px', borderTop:`1px solid ${C.sep}` }}>
                  <span style={{ fontSize:15, fontWeight:700, color: C.orange }}>{t.dashboard.overdue}</span>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={C.orange} strokeWidth="2">
                    <circle cx="12" cy="12" r="9"/><polyline points="12,7 12,12 15,15"/>
                  </svg>
                </div>
                {overdue.map(p => <ReviewRow key={p.id} page={p} isOverdue={true} router={router} t={t} getLastLabel={getLastLabel}/>)}
              </>
            )}
          </>
        )}
      ...
      <div style={{
        ...
        <span style={{ fontSize:13, color: C.sub, marginTop:8 }}>{t.dashboard.addPage}</span>
        <style>{`
      ...
      <div style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'52px 18px 18px', borderBottom:`1px solid ${C.sep}` }}>
          <span style={{ fontSize:16, fontWeight:700, color: C.title }}>{t.dashboard.menu}</span>
        ...
        <div style={{ flex:1, padding:'8px 0' }}>
          {[
            { icon:'📅', label: t.dashboard.upcomingSessions, desc: t.dashboard.upcomingDesc, action:() => { setMenuOpen(false); router.push('/calendar') } },
            { icon:'👤', label: t.dashboard.account,          desc: t.dashboard.accountDesc,  action:() => { setMenuOpen(false); router.push('/account')  } },
          ].map((item, i) => (
      ...
      {toast && (
        <div style={{
          ...
          <div style={{ fontSize:14, fontWeight:700, color: C.green }}>{t.dashboard.pageAdded}</div>
          <div style={{ fontSize:12, color: C.sub, marginTop:3 }}>{t.dashboard.pageAddedSub}</div>
        </div>
      )}
    </div>
  )
...
function ReviewRow({ page, isOverdue, router, t, getLastLabel }: {
  page: Page
  isOverdue: boolean
  router: ReturnType<typeof useRouter>
  t: T
  getLastLabel: (page: Page) => { label: string; color: string }
}) {
  ...
  return (
    <div onClick={() => router.push(`/pages/${page.id}`)} style={{
      ...
      }}>{t.dashboard.reviewBtn}</button>
      <span style={{ fontSize:19, fontWeight:700, color: C.title }}>{t.dashboard.pageLabel(page.page_number)}</span>
